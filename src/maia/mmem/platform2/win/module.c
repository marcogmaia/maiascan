...
lm_bool_t LM_CALL LM_EnumModulesEx(
...
lm_bool_t LM_CALL LM_LoadModule(lm_string_t path, lm_module_t* module_out) {
  ...
  /* TODO: Get library information through the HMODULE returned by LoadLibraryW
   * (should be faster) */

  if (module_out) {
    lm_char_t* name;

    /* NOTE: We search by name instead of path because the path can be
     * misleading, having for example `..\` and similar */
    name = strrchr(path, '\\');
...
lm_bool_t LM_CALL LM_LoadModuleEx(const lm_process_t* process,
                                  lm_string_t path,
  ...
  hproc = open_process(process->pid,
  ...
  hthread = (HANDLE)CreateRemoteThread(hproc,
                                       NULL,
                                       0,
...
lm_bool_t LM_CALL LM_UnloadModule(const lm_module_t* module) {
  HMODULE hmod;
  WCHAR wpath[LM_PATH_MAX];
...
lm_bool_t LM_CALL LM_UnloadModuleEx(const lm_process_t* process,
