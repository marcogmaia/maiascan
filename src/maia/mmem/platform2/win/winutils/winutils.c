...
char* wcstoutf8(WCHAR* widestr, char* utf8buf, size_t buflen) {
  char* cvt = utf8buf;

  /* Either the UTF-8 buffer is NULL (string will be allocated), or it's not
   * NULL and buflen > 0 */
  ...
  /* This function automatically inserts the NULL terminator when passing -1 to
   * 'cchWideChar' */
  if (WideCharToMultiByte(CP_UTF8, 0, widestr, -1, cvt, buflen, NULL, NULL) ==
      0) {
 ...
 *       string dynamically. It must be 'free'd by the caller. */
WCHAR* utf8towcs(char* utf8str, WCHAR* wcsbuf, size_t buflen) {
  WCHAR* cvt = wcsbuf;
...
size_t get_system_bits() {
  size_t bits =
      sizeof(void*) * 8; /* Assume system bits == process bits by default */
...
BOOL get_process_start_time(HANDLE hproc, uint64_t* timestamp_out) {
  ...
  if (NtQuerySystemInformation(
          SystemTimeOfDayInformation, &systime, sizeof(systime), NULL) !=
      STATUS_SUCCESS) {
    return FALSE;
  }
...
BOOL enum_process_entries(BOOL (*callback)(PROCESSENTRY32W* entry, void* arg),
                          void* arg) {
  BOOL result = FALSE;
  HANDLE hsnap;
