...
/* TODO: Perhaps make abstraction for getting a 'lm_process_t' with a
 * PROCESSENTRY32W, since that happens pretty often */
...
BOOL enum_processes_callback(PROCESSENTRY32W* entry, void* arg) {
...
lm_bool_t LM_CALL LM_EnumProcesses(
  ...
  return enum_process_entries(enum_processes_callback, (void*)&parg) ? LM_TRUE
                                                                     : LM_FALSE;
}
...
BOOL get_process_entry_callback(PROCESSENTRY32W* entry, void* arg) {
...
lm_bool_t get_process_entry(lm_pid_t pid, PROCESSENTRY32W* entry) {
...
lm_bool_t LM_CALL LM_GetProcess(lm_process_t* process_out) {
  WCHAR path[MAX_PATH + 1] = {0};
  PROCESSENTRY32W entry;
  ...
  if (!wcstoutf8(
          entry.szExeFile, process_out->name, sizeof(process_out->name))) {
    return LM_FALSE;
  }
...
lm_bool_t LM_CALL LM_GetProcessEx(lm_pid_t pid, lm_process_t* process_out) {
...
lm_char_t** LM_CALL LM_GetCommandLine(lm_process_t* process) {
  LPWSTR wcmdline;
  LPWSTR* wcmdargs;
  int argc;
  size_t len = 0;
  ...
  for (i = 0; i < argc; ++i) {
    len = wcslen(wcmdargs[i]) +
          1;  // we will include the null terminator in the length
    ptr = cmdline;
...
lm_size_t LM_CALL LM_GetSystemBits() {
