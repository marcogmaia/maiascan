...
lm_arch_t get_architecture();

lm_size_t generate_hook_payload(lm_address_t from,
                                lm_address_t to,
                                lm_size_t bits,
                                lm_byte_t** payload_out);

lm_size_t generate_no_ops(lm_byte_t* buf, lm_size_t size);
...
lm_size_t get_max_hook_size();

/* NOTE: This function does heavy assumptions about processes running in a
 * different bitsize than the current process. More testing should be done to
 * check if it actually holds. */
static inline lm_arch_t get_architecture_from_bits(lm_size_t bits) {
  lm_arch_t arch;
