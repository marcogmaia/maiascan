...
LM_API lm_bool_t LM_CALL LM_EnumSegments(
  ...
  for (address = 0; VirtualQuery(address, &meminfo, sizeof(meminfo)) > 0;
    ...
    segment.prot = get_prot(meminfo.Protect);
...
LM_API lm_bool_t LM_CALL LM_EnumSegmentsEx(
  ...
  /* TODO: Add fix for 32 bit processes enumerating 64 bit target processes
