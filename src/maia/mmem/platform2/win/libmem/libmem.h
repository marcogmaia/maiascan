...
#define LM_PID_BAD                                                       \
  ((lm_pid_t) - 1) /* PID 0 is valid, so can't be used here. -1 could be \
                      valid, but it's unlikely */
#define LM_TID_BAD ((lm_tid_t) - 1)
#define LM_ADDRESS_BAD \
  ((lm_address_t) - 1) /* Both 0 and -1 are a good idea here */
...
typedef enum { LM_FALSE = 0, LM_TRUE = 1 } lm_bool_t;
...
typedef struct lm_process_t {
  ...
  lm_size_t bits;
  lm_time_t
      start_time; /* Process start timestamp, in milliseconds since last boot */
...
/**
 * Enumerates processes on a system and calls a callback function for each
 * process found.
 *
 ...
 */
LM_API lm_bool_t LM_CALL LM_EnumProcesses(
    lm_bool_t(LM_CALL* callback)(lm_process_t* process, lm_void_t* arg),
    lm_void_t* arg);
 ...
 * populated with information about the current process.
 *
 * @return `LM_TRUE` if the process information was successfully
 * retrieved or `LM_FALSE` if there was an error.
 */
LM_API lm_bool_t LM_CALL LM_GetProcess(lm_process_t* process_out);
 ...
 * retrieval process.
 */
LM_API lm_bool_t LM_CALL LM_GetProcessEx(lm_pid_t pid,
 ...
 * if it fails.
 */
LM_API lm_char_t** LM_CALL LM_GetCommandLine(lm_process_t* process);
 ...
 */
LM_API lm_void_t LM_CALL LM_FreeCommandLine(lm_char_t** cmdline);
 ...
 */
LM_API lm_bool_t LM_CALL LM_FindProcess(lm_string_t process_name,
                                        lm_process_t* process_out);
 ...
 */
LM_API lm_bool_t LM_CALL LM_IsProcessAlive(const lm_process_t* process);
 ...
 */
LM_API lm_size_t LM_CALL LM_GetBits();
 ...
 */
LM_API lm_size_t LM_CALL LM_GetSystemBits();
 ...
 */
LM_API lm_bool_t LM_CALL LM_EnumThreads(
 ...
 * operation.
 */
LM_API lm_bool_t LM_CALL LM_EnumThreadsEx(
 ...
 *
 * @param thread_out A pointer to the `lm_thread_t` structure that will be
 * populated with information about the current thread, specifically the thread
 * ID (`tid`) and the process ID (`owner_pid`).
 *
 ...
 */
LM_API lm_bool_t LM_CALL LM_GetThread(lm_thread_t* thread_out);

/**
 * Retrieves information about a thread in a process.
 *
 * @param process The process that the thread will be retrieved from.
 * @param thread_out A pointer to the `lm_thread_t` variable where the function
 * will store the thread information retrieved from the process.
 *
 * @return `LM_TRUE` if the thread was retrieved successfully, or
 * `LM_FALSE` if it fails.
 */
LM_API lm_bool_t LM_CALL LM_GetThreadEx(const lm_process_t* process,
 ...
 * @param process_out A pointer to the `lm_process_t` structure where the
 * function `LM_GetThreadProcess` will store the process information related to
 * the given thread.
 *
 * @return `LM_TRUE` if the operation was successful or `LM_FALSE`
 * otherwise.
 */
LM_API lm_bool_t LM_CALL LM_GetThreadProcess(const lm_thread_t* thread,
                                             lm_process_t* process_out);
 ...
 * @param arg An extra argument that is passed to the callback function. This
 * allows you to provide additional information or context to the callback
 * function when it is invoked during the enumeration of modules.
 *
 * @return Returns `LM_TRUE` if the enumeration succeeds, or `LM_FALSE` if it
 * fails.
 */
LM_API lm_bool_t LM_CALL LM_EnumModules(
    lm_bool_t(LM_CALL* callback)(lm_module_t* module, lm_void_t* arg),
    lm_void_t* arg);
 ...
 * fails.
 */
LM_API lm_bool_t LM_CALL LM_EnumModulesEx(
    const lm_process_t* process,
    lm_bool_t(LM_CALL* callback)(lm_module_t* module, lm_void_t* arg),
    lm_void_t* arg);

/**
 * Finds a module by name and populates the `module_out` parameter with the
 ...
 * @param module_out A pointer to a `lm_module_t` structure. This function
 * populates this structure with information about the found module, containing
 * information such as base, end, size, path and name.
 *
 * @return Returns `LM_TRUE` if the module is found successfully, otherwise it
 * returns `LM_FALSE`.
 */
LM_API lm_bool_t LM_CALL LM_FindModule(lm_string_t name,
                                       lm_module_t* module_out);

/**
 * Finds a module by name in a specified process and populates the `module_out`
 * parameter with the found module information.
 *
 ...
 * returns `LM_FALSE`.
 */
LM_API lm_bool_t LM_CALL LM_FindModuleEx(const lm_process_t* process,
                                         lm_string_t name,
                                         lm_module_t* module_out);
 ...
 * @param module_out A pointer to a `lm_module_t` type, which is used to store
 * information about the loaded module (optional).
 *
 * @return Returns `LM_TRUE` is the module was loaded successfully, or
 * `LM_FALSE` if it fails.
 */
LM_API lm_bool_t LM_CALL LM_LoadModule(lm_string_t path,
                                       lm_module_t* module_out);
 ...
 * `LM_FALSE` if it fails.
 */
LM_API lm_bool_t LM_CALL LM_LoadModuleEx(const lm_process_t* process,
                                         lm_string_t path,
                                         lm_module_t* module_out);
 ...
 *
 * @return Returns `LM_TRUE` if the module was successfully unloaded, and
 * `LM_FALSE` if it fails.
 */
LM_API lm_bool_t LM_CALL LM_UnloadModule(const lm_module_t* module);
 ...
 * `LM_FALSE` if it fails.
 */
LM_API lm_bool_t LM_CALL LM_UnloadModuleEx(const lm_process_t* process,
                                           const lm_module_t* module);

/* Symbol API */

/**
 * Enumerates symbols in a module and calls a callback function for each symbol
 * found.
 *
 * @param module The module where the symbols will be enumerated from.
 * @param callback A function pointer that will receive each symbol in the
 * enumeration and an extra argument. The callback function should return
 * `LM_TRUE` to continue the enumeration or `LM_FALSE` to stop it.
 * @param arg A pointer to user-defined data that can be passed to the callback
 * function. It allows you to provide additional information or context.
 *
 * @return Returns `LM_TRUE` if the enumeration succeeds, `LM_FALSE` otherwise.
 */
LM_API lm_bool_t LM_CALL LM_EnumSymbols(
    const lm_module_t* module,
    lm_bool_t(LM_CALL* callback)(lm_symbol_t* symbol, lm_void_t* arg),
    lm_void_t* arg);
 ...
 */
LM_API lm_address_t LM_CALL LM_FindSymbolAddress(const lm_module_t* module,
 ...
 * @param demangled_buf The buffer where the demangled symbol name will be
 * stored. If this is `NULL`, the symbol will be dynamically allocated and
 * `maxsize` is ignored.
 * @param maxsize The maximum size of the buffer where the demangled symbol name
 * will be stored.
 *
 * @return Returns a pointer to the demangled symbol string, or `NULL` if it
 * fails. If the symbol was dynamically allocated, you need to free it with
 * `LM_FreeDemangledSymbol`.
 */
LM_API lm_char_t* LM_CALL LM_DemangleSymbol(lm_string_t symbol_name,
 ...
 */
LM_API lm_void_t LM_CALL LM_FreeDemangledSymbol(lm_char_t* symbol_name);

/**
 * Enumerates symbols in a module with demangled names and calls a provided
 * callback function for each symbol found.
 *
 * @param module The module where the symbols will be enumerated from.
 * @param callback A function pointer that will receive each demangled symbol in
 * the enumeration and an extra argument. The callback function should return
 * `LM_TRUE` to continue the enumeration or `LM_FALSE` to stop it.
 * @param arg A pointer to user-defined data that can be passed to the callback
 * function. It allows you to provide additional information or context.
 *
 * @return Returns `LM_TRUE` if the enumeration succeeds, `LM_FALSE` otherwise.
 */
LM_API lm_bool_t LM_CALL LM_EnumSymbolsDemangled(
    const lm_module_t* module,
    lm_bool_t(LM_CALL* callback)(lm_symbol_t* symbol, lm_void_t* arg),
    lm_void_t* arg);
 ...
 * @return Returns the address of the symbol, or `LM_ADDRESS_BAD` if it fails.
 */
LM_API lm_address_t LM_CALL LM_FindSymbolAddressDemangled(
    const lm_module_t* module, lm_string_t symbol_name);

/* Segment API */

/**
 * Enumerates the memory segments of the current process and invokes a callback
 * function for each segment.
 *
 * @param callback A function pointer that will receive each segment in the
 * enumeration and an extra argument. The callback function should return
 * `LM_TRUE` to continue the enumeration or `LM_FALSE` to stop it.
 * @param arg A pointer to user-defined data that can be passed to the callback
 * function. It allows you to provide additional information or context to the
 * callback function when iterating over segments.
 *
 * @return The function returns `LM_TRUE` if the enumeration was successful, or
 * `LM_FALSE` otherwise.
 */
LM_API lm_bool_t LM_CALL LM_EnumSegments(
    lm_bool_t(LM_CALL* callback)(lm_segment_t* segment, lm_void_t* arg),
    lm_void_t* arg);

/**
 * Enumerates the memory segments of a given process and invokes a callback
 * function for each segment.
 *
 * @param process A pointer to a structure containing information about the
 * process whose segments will be enumerated.
 * @param callback A function pointer that will receive each segment in the
 * enumeration and an extra argument. The callback function should return
 * `LM_TRUE` to continue the enumeration or `LM_FALSE` to stop it.
 * @param arg A pointer to user-defined data that can be passed to the callback
 ...
 * `LM_FALSE` otherwise.
 */
LM_API lm_bool_t LM_CALL LM_EnumSegmentsEx(
    const lm_process_t* process,
    lm_bool_t(LM_CALL* callback)(lm_segment_t* segment, lm_void_t* arg),
    lm_void_t* arg);

/**
 * The function `LM_FindSegment` searches for a memory segment that a given
 * address is within and populates the `segment_out` parameter with the result.
 *
 * @param address The address to search for.
 * @param segment_out A pointer to an `lm_segment_t` structure to populate with
 * information about the segment that contains the specified address.
 *
 * @return The function returns `LM_TRUE` if the specified address is found
 * within a segment, or `LM_FALSE` otherwise.
 */
LM_API lm_bool_t LM_CALL LM_FindSegment(lm_address_t address,
 ...
 *
 * @param process A pointer to a structure containing information about the
 * process whose memory segments will be searched.
 * @param address The address to search for.
 * @param segment_out A pointer to an `lm_segment_t` structure to populate with
 * information about the segment that contains the specified address.
 *
 ...
 * within a segment, or `LM_FALSE` otherwise.
 */
LM_API lm_bool_t LM_CALL LM_FindSegmentEx(const lm_process_t* process,
                                          lm_address_t address,
                                          lm_segment_t* segment_out);
 ...
 */
LM_API lm_size_t LM_CALL LM_ReadMemory(lm_address_t source,
                                       lm_byte_t* dest,
                                       lm_size_t size);
 ...
 */
LM_API lm_size_t LM_CALL LM_ReadMemoryEx(const lm_process_t* process,
                                         lm_address_t source,
                                         lm_byte_t* dest,
                                         lm_size_t size);
 ...
 * address.
 */
LM_API lm_size_t LM_CALL LM_WriteMemory(lm_address_t dest,
 ...
 */
LM_API lm_size_t LM_CALL LM_WriteMemoryEx(const lm_process_t* process,
                                          lm_address_t dest,
                                          lm_bytearray_t source,
                                          lm_size_t size);
 ...
 * `dest`.
 */
LM_API lm_size_t LM_CALL LM_SetMemory(lm_address_t dest,
                                      lm_byte_t byte,
                                      lm_size_t size);
 ...
 */
LM_API lm_size_t LM_CALL LM_SetMemoryEx(const lm_process_t* process,
                                        lm_address_t dest,
                                        lm_byte_t byte,
                                        lm_size_t size);

/**
 * The function sets memory protection flags for a specified memory address
 * range.
 *
 ...
 * @param prot The new protection flags that will be applied to the memory
 * region starting at the specified address. It is a bit mask of `LM_PROT_X`
 * (execute), `LM_PROT_R` (read), `LM_PROT_W` (write).
 * @param oldprot_out A pointer to a `lm_prot_t` type variable that will be used
 * to store the old protection flags of a memory segment before they are updated
 * with the new protection settings specified by the `prot` parameter.
 *
 * @return The function returns a boolean value, either `LM_TRUE` or `LM_FALSE`,
 * based on the success of the memory protection operation.
 */
LM_API lm_bool_t LM_CALL LM_ProtMemory(lm_address_t address,
                                       lm_size_t size,
                                       lm_prot_t prot,
                                       lm_prot_t* oldprot_out);

/**
 * The function modifies memory protection flags for a specified address range
 * in a given process.
 *
 * @param process A pointer to the process that the memory flags will be
 * modified from.
 * @param address The memory address to be protected.
 * @param size The size of memory to be protected. If the size is 0,
 * the function will default to using the system's page size for the operation.
 ...
 */
LM_API lm_bool_t LM_CALL LM_ProtMemoryEx(const lm_process_t* process,
                                         lm_address_t address,
                                         lm_size_t size,
                                         lm_prot_t prot,
                                         lm_prot_t* oldprot_out);
 ...
 */
LM_API lm_address_t LM_CALL LM_AllocMemory(lm_size_t size, lm_prot_t prot);
 ...
 * `LM_ADDRESS_BAD`.
 */
LM_API lm_address_t LM_CALL LM_AllocMemoryEx(const lm_process_t* process,
                                             lm_size_t size,
                                             lm_prot_t prot);
 ...
 */
LM_API lm_bool_t LM_CALL LM_FreeMemory(lm_address_t alloc, lm_size_t size);

/**
 * The function deallocates memory that was previously allocated with
 * `LM_AllocMemoryEx` on a given process.
 *
 * @param process A pointer to the process that the memory will be deallocated
 * from.
 * @param alloc The address of the memory block that was previously allocated
 ...
 * not.
 */
LM_API lm_bool_t LM_CALL LM_FreeMemoryEx(const lm_process_t* process,
                                         lm_address_t alloc,
                                         lm_size_t size);
 ...
 * @param offsets An array of offsets used to navigate through the memory
 * addresses.
 ...
 */
LM_API lm_address_t LM_CALL LM_DeepPointer(lm_address_t base,
 ...
 *
 * @param process A pointer to the process that the deep pointer will be
 * calculated from.
 * @param base The starting address from which to calculate the deep pointer.
 * @param offsets An array of offsets used to navigate through the memory
 * addresses.
 ...
 */
LM_API lm_address_t LM_CALL LM_DeepPointerEx(const lm_process_t* process,
                                             lm_address_t base,
                                             const lm_address_t* offsets,
                                             lm_size_t noffsets);
 ...
 * `LM_ADDRESS_BAD`.
 */
LM_API lm_address_t LM_CALL LM_DataScan(lm_bytearray_t data,
                                        lm_size_t datasize,
                                        lm_address_t address,
                                        lm_size_t scansize);
 ...
 * `LM_ADDRESS_BAD`.
 */
LM_API lm_address_t LM_CALL LM_DataScanEx(const lm_process_t* process,
                                          lm_bytearray_t data,
                                          lm_size_t datasize,
                                          lm_address_t address,
                                          lm_size_t scansize);
 ...
 */
LM_API lm_address_t LM_CALL LM_PatternScan(lm_bytearray_t pattern,
                                           lm_string_t mask,
                                           lm_address_t address,
                                           lm_size_t scansize);

/**
 * The function searches for a specific pattern in memory in a given process
 * based on a mask.
 *
 * @param process The process whose memory will be scanned.
 * @param pattern The pattern to be searched for in memory.
 * @param mask The pattern mask used for scanning memory. It is used to specify
 * which bytes in the pattern should be matched against the memory content. The
 * mask can contain characters such as
 * '?' which act as wildcards, allowing any byte to be matched. You can also use
 * 'x' to have an exact match.
 * @param address The starting memory address where the scanning operation will
 * begin. The function will scan the memory starting from this address to find
 * the pattern match.
 * @param scansize The size of the memory region to scan starting from the
 * specified `address`. It determines the range within which the function will
 * search for the specified pattern based on the provided `pattern` and `mask`.
 *
 * @return The function returns the memory address where a match for the given
 * pattern and mask is found within the specified scan size starting from the
 * provided address. If no match is found or if an error occurs, the function
 * returns `LM_ADDRESS_BAD`.
 */
LM_API lm_address_t LM_CALL LM_PatternScanEx(const lm_process_t* process,
...
/**
 * The function searches for a specific signature pattern in memory starting
 * from a given address within a specified scan size.
 *
 * @param signature The signature to be scanned for in memory. It is used to
 * identify a specific pattern of bytes in memory. You can use `??` to match
 * against any byte, or the byte's hexadecimal value. Example: `"DE AD BE EF ??
 * ?? 13 37"`.
 * @param address The starting memory address where the signature scanning will
 * begin. The function will scan the memory starting from this address to find
 * the pattern match.
 * @param scansize The size of the memory region to scan starting from the
 * `address` parameter. It specifies the number of bytes to search for the
 * signature pattern within the memory region.
 *
 * @return The function retuns either the address of the pattern match found in
 * the specified memory range or `LM_ADDRESS_BAD` if no match is found (or an
 * error occurs).
 */
LM_API lm_address_t LM_CALL LM_SigScan(lm_string_t signature,
                                       lm_address_t address,
                                       lm_size_t scansize);

/**
 * The function searches for a specific signature pattern in memory from a given
 * process starting from a specific address within a specified scan size.
 *
 * @param process The process whose memory will be scanned.
 * @param signature The signature to be scanned for in memory. It is used to
 * identify a specific pattern of bytes in memory. You can use `??` to match
 ...
 * error occurs).
 */
LM_API lm_address_t LM_CALL LM_SigScanEx(const lm_process_t* process,
 ...
 *
 * @return The function returns the architecture of the system. It can be one
 * of:
 * - `LM_ARCH_X86` for 32-bit x86.
 * - `LM_ARCH_X64` for 64-bit x86.
 * - Others (check the enum for `lm_arch_t`)
 */
LM_API lm_arch_t LM_CALL LM_GetArchitecture();
 ...
 * Example: `"mov eax, ebx"`.
 * @param instruction_out The assembled instruction is populated into this
 * parameter.
 *
 * @return The function returns `LM_TRUE` if it succeeds in assembling the
 * instruction, and populates the `instruction_out` parameter with the assembled
 * instruction. If the instruction could not be assembled successfully, then the
 * function returns `LM_FALSE`.
 */
LM_API lm_bool_t LM_CALL LM_Assemble(lm_string_t code,
 ...
 * the addressing (for example, relative jumps will be resolved using this
 * address).
 * @param payload_out A pointer to the buffer that will receive the assembled
 * instructions. The buffer should be freed with `LM_FreePayload` after use.
 *
 * @return On success, it returns the size of the assembled instructions, in
 * bytes. On failure, it returns `0`.
 */
LM_API lm_size_t LM_CALL LM_AssembleEx(lm_string_t code,
                                       lm_arch_t arch,
 ...
 */
LM_API lm_void_t LM_CALL LM_FreePayload(lm_byte_t* payload);
 ...
 * @param instruction_out The disassembled instruction is populated into this
 * parameter.
 *
 * @return `LM_TRUE` on success, `LM_FALSE` on failure.
 */
LM_API lm_bool_t LM_CALL LM_Disassemble(lm_address_t machine_code,
                                        lm_inst_t* instruction_out);
 ...
 * the addressing (for example, relative jumps will be resolved using this
 * address).
 * @param instructions_out A pointer to the buffer that will receive the
 * disassembled instructions. The buffer should be freed with
 * `LM_FreeInstructions` after use.
 *
 * @return On success, it returns the count of the instructions disassembled. On
 * failure, it returns `0`.
 */
LM_API lm_size_t LM_CALL LM_DisassembleEx(lm_address_t machine_code,
                                          lm_arch_t arch,
                                          lm_size_t max_size,
...
/**
 * The function deallocates the memory allocated by `LM_DisassembleEx` for the
 * disassembled instructions.
 *
 * @param instructions The disassembled instructions allocated by
 * `LM_DisassembleEx`.
 */
LM_API lm_void_t LM_CALL LM_FreeInstructions(lm_inst_t* instructions);

/**
 * The function calculates the size aligned to the instruction length, based on
 * a minimum size.
 *
 * @param machine_code The address of the instructions.
 * @param min_length The minimum size to be aligned to instruction length.
 *
 * @return On success, it returns the aligned size to the next instruction's
 * length. On failure, it returns `0`.
 */
LM_API lm_size_t LM_CALL LM_CodeLength(lm_address_t machine_code,
                                       lm_size_t min_length);

/**
 * The function calculates the size aligned to the instruction length, based on
 * a minimum size, in a remote process.
 *
 ...
 * length. On failure, it returns `0`.
 */
LM_API lm_size_t LM_CALL LM_CodeLengthEx(const lm_process_t* process,
...
/**
 * The function places a hook/detour onto the address `from`, redirecting it to
 * the address `to`. Optionally, it generates a trampoline in `trampoline_out`
 ...
 * @param trampoline_out Optional pointer to an `lm_address_t` variable that
 * will receive a trampoline/gateway to call the original function.
 *
 * @return The amount of bytes occupied by the hook (aligned to the nearest
 * instruction).
 */
LM_API lm_size_t LM_CALL LM_HookCode(lm_address_t from,
                                     lm_address_t to,
                                     lm_address_t* trampoline_out);

/**
 * The function places a hook/detour onto the address `from` in a remote
 * process, redirecting it to the address `to`. Optionally, it generates a
 * trampoline in `trampoline_out` to call the original function in the remote
 * process.
 *
 ...
 * @param trampoline_out Optional pointer to an `lm_address_t` variable that
 * will receive a trampoline/gateway to call the original function in the remote
 * process.
 *
 * @return The amount of bytes occupied by the hook (aligned to the nearest
 * instruction) in the remote process.
 */
LM_API lm_size_t LM_CALL LM_HookCodeEx(const lm_process_t* process,
                                       lm_address_t from,
                                       lm_address_t to,
                                       lm_address_t* trampoline_out);

/**
 * The function removes a hook/detour placed on the address `from`, restoring it
 * to its original state. The function also frees the trampoline allocated by
 * `LM_HookCode`.
 *
 * @param from The address where the hook was placed.
 * @param trampoline The address of the trampoline generated by `LM_HookCode`.
 * @param size The amount of bytes occupied by the hook (aligned to the nearest
 * instruction).
 *
 * @return `LM_TRUE` on success, `LM_FALSE` on failure.
 */
LM_API lm_bool_t LM_CALL LM_UnhookCode(lm_address_t from,
                                       lm_address_t trampoline,
                                       lm_size_t size);

/**
 * The function removes a hook/detour placed on the address `from` in a remote
 * process, restoring it to its original state. The function also frees the
 * trampoline allocated by `LM_HookCodeEx`.
 *
 ...
 * @return `LM_TRUE` on success, `LM_FALSE` on failure.
 */
LM_API lm_bool_t LM_CALL LM_UnhookCodeEx(const lm_process_t* process,
                                         lm_address_t from,
                                         lm_address_t trampoline,
                                         lm_size_t size);
 ...
 * @param vmt_out A pointer to the VMT manager that will be populated by this
 * function.
 *
 * @return On success, it returns `LM_TRUE`. On failure, it returns `LM_FALSE`.
 */
LM_API lm_bool_t LM_CALL LM_VmtNew(lm_address_t* vtable, lm_vmt_t* vmt_out);

/**
 * The function hooks the VMT function at index `from_fn_index` in the VMT
 * managed by `vmt`, changing it to `to`.
 *
 ...
 * @return On success, it returns `LM_TRUE`. On failure, it returns `LM_FALSE`.
 */
LM_API lm_bool_t LM_CALL LM_VmtHook(lm_vmt_t* vmt,
                                    lm_size_t from_fn_index,
                                    lm_address_t to);

/**
 * The function unhooks the VMT function at index `fn_index` in the VMT managed
 * by `vmt`, restoring the original function.
 *
 * @param vmt The VMT manager.
 * @param fn_index The index of the VMT function to unhook.
 */
LM_API lm_bool_t LM_CALL LM_VmtUnhook(lm_vmt_t* vmt, lm_size_t fn_index);

/**
 * The function returns the original VMT function at index `fn_index` in the VMT
 * managed by `vmt`. If the function has not been hooked before, it returns the
 * function pointer at that index in the VMT array.
 *
 * @param vmt The VMT manager.
 * @param fn_index The index of the VMT function to query.
 *
 * @return The original VMT function at index `fn_index` in the VMT managed by
 * `vmt`.
 */
LM_API lm_address_t LM_CALL LM_VmtGetOriginal(const lm_vmt_t* vmt,
                                              lm_size_t fn_index);
 ...
 * @param vmt The VMT manager.
 */
LM_API lm_void_t LM_CALL LM_VmtReset(lm_vmt_t* vmt);
 ...
 * @param vmt The VMT manager.
 */
LM_API lm_void_t LM_CALL LM_VmtFree(lm_vmt_t* vmt);
