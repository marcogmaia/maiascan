...
#elif defined(__GNUC__) || defined(__clang__)
// Ref:
// https://gcc.gnu.org/onlinedocs/gcc/x86-Built-in-Functions.html#index-_005f_005fcpuid
inline void Cpuid(int info[4], int function_id) {
  __cpuid(function_id, info[0], info[1], info[2], info[3]);
}

// Ref:
// https://gcc.gnu.org/onlinedocs/gcc/x86-Built-in-Functions.html#index-_005f_005fcpuid_005fcount
inline void CpuidEx(int info[4], int function_id, int subfunction_id) {
  __cpuid_count(
...
inline bool HasAvx2() {
  ...
  internal::Cpuid(cpu_info.data(), 0);
  ...
  internal::CpuidEx(cpu_info.data(), 7, 0);
