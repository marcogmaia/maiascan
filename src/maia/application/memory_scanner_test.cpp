...
//     GetMemoryRegions()).WillRepeatedly(Return(regions_));
...
//               std::memcpy(buffer.data(), &memory_data1[offset],
//               sizeof(float)); return true;
//             }
//           } else if (addr >= 0x2000 && addr < 0x3000) {
//             size_t offset = addr - 0x2000;
//             if (offset + sizeof(float) <= memory_data2.size()) {
//               std::memcpy(buffer.data(), &memory_data2[offset],
//               sizeof(float)); return true;
//             }
//           }
//         }
...
//   // Initialize both regions with values == 100 (so they won't match
//   kNotEqual) for (size_t i = 0; i < memory_data1.size() / sizeof(uint64_t);
//   ++i) {
...
//   auto first_params = MakeScanParams<uint32_t>(ScanComparison::kExactValue,
//   20); auto first_result = scanner.NewScan(first_params);
